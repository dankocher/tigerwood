...
export default class ButtonArrow extends React.Component {
    ...
    render() {
        const {className, topText, bottomText, width, onClick, hideArrow} = this.props;
        const arrowWidth = this.props.arrowWidth || 86;

        const onlyOneText = bottomText ? {} : {
        ...
        return ( <div className={`-arrow-button ${className}`} style={{width, ...onlyOneText}} onClick={onClick}>
            <div className="b-top-text">{topText}</div>
            {!bottomText ? null :
                <div className="b-bottom-text">{bottomText}
                    {hideArrow ? null :
                        <div className={"arrow"}>
                            <svg width={arrowWidth} height="8" viewBox={`0 0 ${arrowWidth} 8`} fill="none"
                                ...
                                <path d={`m${arrowWidth - 4} 7 3-3-3-3`} fill="none" stroke="#fff" strokeLinecap="round"
                                      strokeLinejoin="round" strokeWidth="1px"/>
